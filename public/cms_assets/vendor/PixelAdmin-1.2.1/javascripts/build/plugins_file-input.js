...
(function() {
  ...
  FileInput = function($input, options) {
    ...
    this.$clear_btn = $(FileInput.DEFAULTS.clear_btn_tmpl).addClass('pfi-clear').appendTo($('.pfi-actions', this.$el));
    this.$choose_btn = $(FileInput.DEFAULTS.choose_btn_tmpl).addClass('pfi-choose').appendTo($('.pfi-actions', this.$el));
    this.onChange();
