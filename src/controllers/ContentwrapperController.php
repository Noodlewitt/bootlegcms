...
class ContentwrapperController extends CMSController
{
    ...
    {
        if (!$id) {
            $this->content = $this->content->with([
                'template_setting',
                'setting',
            ])->fromApplication()->whereNull('parent_id')->first();
        } else {
            $this->content = $this->content->with(['template_setting', 'setting'])->findOrFail($id);
        }
    ...
    public function anyCreate($parent_id = null)
    {
        ...
        $input = $content->loadDefaultValues(['parent_id' => $parent_id, 'template_id' => \Input::get('template_id')]);
        ...
        if (\Request::ajax()) {
            ...
            return $this->render('layouts.large',
                compact('content', 'children', 'childrenSettings', 'settings', 'allPermissions', 'children'));
        }
    }
    ...
    public function anyStore()
    {
        ...
        if ($validation->passes()) {
            Event::fire('content.create', [$this->content]);
            Event::fire('content.update', [$this->content]);
            $content = $this->content->superSave($input);
            $content = ContentwrapperController::saveSettings($content, $input);
            Event::fire('content.created', [$content]);
            Event::fire('content.updated', [$content]);
            //  dd($tree);

            return \Redirect::action(@$content->edit_action ? $content->edit_action : '\Bootleg\Cms\ContentsController@anyEdit',
                $content->id);
    ...
    public function anyTreeStore()
    {
        ...
        if ($validation->passes()) {
            ...
            Event::fire('content.created', [$tree]);
            Event::fire('content.updated', [$tree]);
            //  dd($tree);
    ...
    {
        $content = $this->content->with(['template_setting', 'setting'])->findOrFail($id);
    ...
    public function anyEdit($id = false)
    {
        ...
        $children = $content->children()->with(['setting', 'template_setting'])->paginate();
        ...
        if (\Request::ajax()) {
            return $this->render($content->edit_view,
                compact('content', 'childrenSettings', 'settings', 'allPermissions'));
        } else {
            $tree = $content->getDescendants(config('bootlegcms.cms_tree_descendents'));
    ...
    public function getSearch($id)
    {
        ...
        if (\Request::ajax()) {
            return $this->render($content->edit_view,
                compact('search', 'content', 'settings', 'children', 'childrenSettings'));
        } else {
            return $this->render('layouts.large',
                compact('search', 'content', 'settings', 'children', 'childrenSettings'));
        }
    ...
    public function anyUpdate($id = false)
    {
        ...
        if ($id !== false) {
            ...
            if ($validation->passes()) {
                ...
                \Event::fire('content.edit', [$content]);
                \Event::fire('content.update', [$content]);
                ...
                //TODO: care with Template settings.
                \Event::fire('content.edited', [$content]);
                \Event::fire('content.updated', [$content]);
                ...
                foreach ($settingGroup as $type => $setGrp) {
                    ...
                    if ($type != '_multi') {
                        ...
                        foreach ($setGrp as $key => $setting) {
                                    ...
                                    if (is_array($set)) {
                                        ...
                                        foreach ($set as $k => $s) {
                                            if ($k == 'deleted') {
                                                ...
                                                //TODO: REMOVE
                                                //dd($k);
                                                \Contentsetting::destroy($key);//tstme
                                            }
                                        }
                                    }
                                }
                            ...
                            if (isset($setting) && is_array($setting) && $deleted == false) {
                                    ...
                                    foreach ($template_set as $index => $set) {
                                        if (isset($set['deleted'])) {
                                            ...
                                            if ($type == "Contentsetting") {
                                                //update existing
                                                $contentSetting = \Contentsetting::where('parent_id',
                                                    $key)->where('index', $index)->first();
    ...
    public function anyTreeJson()
    {
        ...
        if (config('bootlegcms.cms_tree_descendents')) {
            $tree = $this->content->where('id', '=',
                $id)->first()->getDescendants(config('bootlegcms.cms_tree_descendents'))->toHierarchy();
        } else {
    ...
    public function anyTableCreate($parent_id = null)
    {
        ...
        $content->parent_id = $parent_id;
        $parent = \Content::findOrFail($parent_id);
        $input = $content->loadDefaultValues(['parent_id' => $parent_id]);
        foreach ($input as $key => $put) {
            $content->$key = $put;
        }
        $allPermissions = [];
        ...
        if (\Request::ajax()) {
            ...
            return $this->render('contents.edit',
                compact('content', 'children', 'childrenSettings', 'settings', 'allPermissions', 'children'));
        }
    }
    ...
    public function getTable($id = null)
    {
        ...
        $children = \Content::with(['template_setting', 'setting'])->where('parent_id', $content->id);
        ...
        if (\Request::ajax()) {
            return $this->render('contents.table.index',
                compact('content', 'children', 'childrenSettings', 'settings', 'children', 'allPermissions'));
        } else {
            return $this->render('layouts.large',
                compact('content', 'children', 'childrenSettings', 'settings', 'allPermissions', 'children'));
        }
    ...
    public function getRenderSetting($id, $content_id, $type = 'Contentsetting')
    {
        ...
        if (strpos($setting->field_type, '::')) {
            return $this->render($setting->field_type, ['setting' => $setting, 'content' => $content]);
        } else {
            return $this->render('contents.input_types.' . $setting->field_type,
                ['setting' => $setting, 'content' => $content]);
        }
            ...
            foreach ($files as $file) {
                ...
                if ($validator->passes()) {
                    ...
                    if (@$this->application->getSetting('Enable s3')) {
                        $finalUrl = S3::upload($fileName,
                            $destinationPath . $uploadFolder . '/' . $fileId . '.' . $extension);
                    }
                    ...
                    $fileObj->deleteUrl = action('\Bootleg\Cms\ContentsController@getDeleteUpload',
                        [$fileName]); //"//".$_SERVER['SERVER_NAME']."/cms/deleteuploads/$fileName"; //todo
                ...
                \Event::fire('upload.complete', [$finalUrl]);
            }
            return response()->json($return);
        }
    }
