...
class ContentwrapperController extends CMSController {
	...
	public function anyTree($id = NULL) {
		if (!$id) {
			$this->content = $this->content->with(['template_setting', 'setting'])->fromApplication()->whereNull('parent_id')->first();
		} else {
	...
	public function getRenderSetting($id, $content_id, $type = 'Contentsetting') {
		...
		if (strpos($setting->field_type, '::')) {
			...
			return $this->render('contents.input_types.' . $setting->field_type, ['setting' => $setting, 'content' => $content]);
		}
		...
		if (!empty($files)) {
			foreach ($files as $file) {
				...
				if ($validator->passes()) {
					...
					$fileObj->deleteUrl = action('\Bootleg\Cms\ContentsController@getDeleteUpload', [$fileName]); //"//".$_SERVER['SERVER_NAME']."/cms/deleteuploads/$fileName"; //todo
