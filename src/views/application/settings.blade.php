        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <ul class="nav nav-tabs">
                    <?php
                        if( ! $application_settings->has('Application')) $application_settings->put('Application', []);
                    ?>
                    @foreach($application_settings as $key => $section)
                        <li class='{{@!$i?"active":""}}'><a href="#tab-{{ $key ? $key : "Application" }}" data-toggle="tab">{{ $key ? $key : "Application" }}</a></li>
                        <?php $i = 1?>
                    @endforeach
                </ul>
                ...
                <div class='tab-content'>
                    @foreach($application_settings as $key=>$section)
                        ...
                        ?>
                        <div class="tab-pane fade {{ ! @$j ? 'active in' : '' }} edit-content-tab"
                             id="tab-{{ $key ? $key : "Application"}}">
                            <ul>
                                <?php $j = true; ?>
                                @if($key == 'Application')
                                    <li class="form-group">
